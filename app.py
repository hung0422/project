...
def process_postback_event(event):
    ...
    # 從資料庫取出用戶個人資料，並建立網頁供用戶修改資料，再存回資料庫
        ...
        # 從資料庫取出用戶個人資料
        sql = '''SELECT u.name,u.gender,r.regionName,u.email from user_info u join region r 
        ...
        message_before = transform.transform_info(data_name, data_gender, data_regionID, email, liff_uri)
        message_after = BubbleContainer.new_from_json_dict(json.loads(message_before))
    ...
    # 推薦商品
        ...
        sql2 = '''select shoppingdate from shoppinglist where userID = '{}' group by shoppingdate;'''.format(userID)
        cursor.execute(sql2)
        data = cursor.fetchall()

        if len(data) <= 3:
            message_before = transform.transform_recom_empty()
            message_after = CarouselContainer.new_from_json_dict(json.loads(message_before))
            message = FlexSendMessage(alt_text="oh2", contents=message_after)
            line_bot_api.reply_message(
                event.reply_token,
                message)
        else:
...
def pushWelcom(userID):
    try:
        line_bot_api.push_message(userID,TextSendMessage(text="歡迎光臨吾人能購!\n點擊'購物車'來查看本次交易的即時購買情形\n祝您購物愉快!"))
    except :
        print('Push Error.')
...
def pushthank(userID):
    try:
        line_bot_api.push_message(userID,TextSendMessage(text="感謝您的光臨!\n點擊'交易紀錄'來查看最新紀錄"))
    except :
        print('Push Error.')
